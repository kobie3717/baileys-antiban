...
await test('mutex error from originalSendMessage propagates — first succeeds, second fails', async () => {
  const mock = createMockSocket({
    sendDelay: 50,
    throwOnConcurrent: true,
    throwError: 'Mutex locked: request already pending',
  });
  const wrapped = wrapSocket(mock, TEST_CONFIG);

  const jid = 'test@s.whatsapp.net';
  const [first, second] = await Promise.allSettled([
    wrapped.sendMessage(jid, { text: 'first' }),
    wrapped.sendMessage(jid, { text: 'second' }),
  ]);

  assert(first.status === 'fulfilled', `First send should succeed, got: ${first.status === 'rejected' ? first.reason?.message : 'ok'}`);
  assert(second.status === 'rejected', 'Second concurrent send should fail with mutex error');
  if (second.status === 'rejected') {
    assert(
      second.reason.message.includes('Mutex locked'),
      `Expected mutex error, got: ${second.reason.message}`
    );
  }
});

await test('failed send not counted as success (messagesAllowed stays correct)', async () => {
  const mock = createMockSocket({ sendDelay: 50, throwOnConcurrent: true });
  const wrapped = wrapSocket(mock, TEST_CONFIG);
  ...
  const after = wrapped.antiban.getStats().messagesAllowed;
  assert(after - before === 1, `Only 1 success should be counted, got ${after - before}`);
});
...
await test('KNOWN LIMITATION: concurrent sends bypass rate limiter (beforeSend/afterSend race)', async () => {
  // When N sends fire simultaneously, all call beforeSend() before any afterSend() commits.
  // This means concurrent sends all see 0 messages sent and all pass the rate limit check.
  // This is a known limitation — rate limiting only works correctly for sequential sends.
  // rc10 mutex redesign does NOT fix this (it's a baileys-antiban architectural issue).
  const mock = createMockSocket();
  const wrapped = wrapSocket(mock, { ...TEST_CONFIG, maxPerMinute: 1 });
  ...
  const succeeded = results.filter(r => r.status === 'fulfilled').length;
  // Document: all 5 bypass the rate limiter because they race
  assert(
    succeeded === 5,
    `Expected all 5 to bypass rate limiter in concurrent mode, got ${succeeded} — behavior changed`
  );
  assert(mock.calls - callsBefore === 5, 'All 5 reached originalSendMessage despite maxPerMinute:1');
  // This test passing means the race condition is still present (expected).
  // If this test FAILS, it means rate limiting now works under concurrency — a welcome fix!
});
