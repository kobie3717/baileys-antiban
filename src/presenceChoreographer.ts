...
const DEFAULT_CONFIG: Required<Omit<PresenceChoreographerConfig, 'circadian'>> & { circadian: Required<NonNullable<PresenceChoreographerConfig['circadian']>> } = {
  enabled: false,
...
export class PresenceChoreographer {
  private config: Required<Omit<PresenceChoreographerConfig, 'circadian'>> & { circadian: Required<NonNullable<PresenceChoreographerConfig['circadian']>> };
  private stats = {
  ...
  shouldMarkRead(): { mark: boolean; delayMs: number } {
    ...
    const baseDelayMs = this.randomBetween(
  ...
  computeTypingPlan(messageLength: number): TypingPlanStep[] {
    ...
    // 5. Clamp to min/max
    const targetMs = this.clamp(
      baseMs * circadianMultiplier,
      this.config.typingMinMs,
    ...
    for (let i = 0; i < numChunks && remainingBudget > 0; i++) {
      ...
      if (i > 0 && i < numChunks - 1 && Math.random() < this.config.thinkPauseProbability) {
        ...
        const basePauseMs = this.randomBetween(
