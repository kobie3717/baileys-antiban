...
export interface InstanceCoordinatorStats {
  ...
  poolMaxPerMinute: number;
  poolMaxPerHour: number;
  poolUtilization: number; // 0–1, fraction of per-minute budget used
...
export class InstanceCoordinator {
  private config: Required<InstanceCoordinatorConfig>;
   ...
   */
  tryAcquireSlot(): { allowed: boolean; retryAfterMs?: number } {
    try {
      const now = Date.now();
  ...
  private readState(): CoordinationState {
    try {
      ...
      const parsed = JSON.parse(content) as CoordinationState;
