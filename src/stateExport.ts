...
export interface RateLimiterState {
  ...
  messages: Array<{ timestamp: number; recipient: string; contentHash: string }>;
  /** Known chat JIDs */
  knownChats: string[];
 ...
 */
export function exportAntibanState(modules: {
  warmup?: { exportState: () => WarmUpState };
  health?: { getStatus: () => any };
  rateLimiter?: {
    getStats: () => any;
  ...
  circuits?: {
    exportState?: () => Array<{ jid: string; state: 'open' | 'half-open' | 'closed'; failures: number; openedAt: number | null; halfOpenProbeUsed?: boolean }>;
  };
  timelockGuard?: {
    getState: () => { isActive: boolean; expiresAt?: Date; enforcementType?: string };
    getKnownChats?: () => Set<string>;
  };
  messageRegistry?: {
  ...
  const snapshot: AntibanSnapshot = {
    version: 1,
    exportedAt: Date.now(),
 ...
 */
export function importAntibanState(
  snapshot: AntibanSnapshot,
  modules: {
    warmup?: { importState?: (state: WarmUpState) => void };
    health?: { reset?: () => void };
    rateLimiter?: {
    ...
    circuits?: {
      importState?: (states: Array<{ jid: string; state: 'open' | 'half-open' | 'closed'; failures: number; openedAt: number | null; halfOpenProbeUsed?: boolean }>) => void;
    };
    timelockGuard?: {
      reset: () => void;
