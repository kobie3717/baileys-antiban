...
export function proxyRotator(config: ProxyRotatorConfig): ProxyRotatorHandle {
  const {
    pool,
  ...
  if (scheduledIntervalMs > 0 && scheduledIntervalMs < 60_000) {
    logger.warn?.(
      `proxyRotator: scheduledIntervalMs (${scheduledIntervalMs}ms) is < 60s. May hammer proxy provider.`
    );
  }

  // Internal state
  const states: EndpointState[] = pool.map((endpoint) => ({
    endpoint,
    failures: 0,
    lastUsedAt: null,
    isDead: false,
  ...
  const rotationsByTrigger: Record<string, number> = {};
  let scheduledTimer: NodeJS.Timeout | null = null;

  // Agent cache: map endpoint -> Agent (cleared on rotation)
  const agentCache = new Map<ProxyEndpoint, Agent>();

  // Lazy-load proxy agent libs
  let socksProxyAgentImport: any = null;
  let httpProxyAgentImport: any = null;
  let httpsProxyAgentImport: any = null;


  function buildProxyUrl(endpoint: ProxyEndpoint): string {
    const { type, host, port, username, password } = endpoint;
    const auth = username && password ? `${username}:${password}@` : '';
    return `${type}://${auth}${host}:${port}`;
  }

  function createAgentForEndpointSync(endpoint: ProxyEndpoint): Agent | null {
    // Check cache first
  ...
  function getAliveEndpoints(): number[] {
    const now = Date.now();
    return states
      .map((s, idx) => {
        ...
        if (s.isDead && s.lastUsedAt) {
          if (now - s.lastUsedAt.getTime() >= deadCooldownMs) {
            ...
            s.failures = 0;
            logger.info?.(
              `Resurrected endpoint ${s.endpoint.label || s.endpoint.host} after cooldown`
            );
          }
        }
  ...
  function selectNextIndex(alive: number[]): number {
    if (alive.length === 0) return currentIndex; // All dead, stay on current
  ...
  function rotateImpl(reason: 'manual' | 'disconnect' | 'ban-warning' | 'scheduled' = 'manual'): ProxyEndpoint | null {
    if (pool.length === 1) {
  ...
  function markFailureImpl(): void {
    const state = states[currentIndex];
    state.failures++;

    const label = state.endpoint.label || state.endpoint.host;
    logger.warn?.(
      `Endpoint ${label} failed (${state.failures}/${maxFailures})`
    );
  ...
  function resurrectAllImpl(): void {
    let count = 0;
  ...
  function getStatsImpl(): ProxyRotatorStats {
    return {
      totalRotations,
      ...
      currentEndpoint:
        states[currentIndex].endpoint.label || states[currentIndex].endpoint.host,
    };
  }

  function currentAgentImpl(): Agent | null {
    const endpoint = states[currentIndex].endpoint;
    return createAgentForEndpointSync(endpoint);
  }

  function currentImpl(): ProxyEndpoint | null {
    return states[currentIndex].endpoint;
  }
  ...
  if (rotateOn.includes('scheduled') && scheduledIntervalMs > 0) {
    ...
    logger.info?.(
      `Scheduled rotation enabled (every ${scheduledIntervalMs}ms)`
    );
  }
