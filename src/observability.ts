 ...
 */
export function createConsoleLogger(prefix: string = '[baileys-antiban]'): AntiBanLogger {
...
export function exportPrometheusMetrics(
  ...
  // Counters
  addMetric(
    'antiban_messages_allowed_total',
    'counter',
    'Total messages allowed by AntiBan',
    stats.messagesAllowed
  );
  addMetric(
    'antiban_messages_blocked_total',
    'counter',
    'Total messages blocked by AntiBan',
    stats.messagesBlocked
  );
  addMetric(
    'antiban_total_delay_ms_total',
    'counter',
    'Total delay imposed (milliseconds)',
    stats.totalDelayMs
  );
  ...
  addMetric(
    'antiban_health_score',
    'gauge',
    'Current health score (0=best, 100=worst)',
    stats.health.score
  );
  addMetric(
    'antiban_health_risk',
    'gauge',
    'Risk level encoded as number (0=low, 1=medium, 2=high, 3=critical)',
    riskMap[stats.health.risk] ?? 0
  );
  addMetric(
    'antiban_disconnects_last_hour',
    'gauge',
    'Disconnects in last hour',
    stats.health.stats.disconnectsLastHour
  );
  addMetric(
    'antiban_failed_messages_last_hour',
    'gauge',
    'Failed messages in last hour',
    stats.health.stats.failedMessagesLastHour
  );
  addMetric(
    'antiban_uptime_ms',
    'gauge',
    'Session uptime in milliseconds',
    stats.health.stats.uptimeMs
  );

  // Warm-up gauges
  addMetric(
    'antiban_warmup_progress',
    'gauge',
    'Warm-up progress (0.0 to 1.0)',
    stats.warmUp.progress
  );
  addMetric(
    'antiban_warmup_day',
    'gauge',
  ...
  addMetric(
    'antiban_warmup_today_sent',
    'gauge',
    'Messages sent today during warm-up',
    stats.warmUp.todaySent
  );
  addMetric(
    'antiban_warmup_today_limit',
    'gauge',
    'Message limit for today during warm-up',
    stats.warmUp.todayLimit
  );

  // Rate limiter gauges
  addMetric(
    'antiban_rate_last_minute',
    'gauge',
    'Messages sent in last minute',
    stats.rateLimiter.lastMinute
  );
  addMetric(
    'antiban_rate_last_hour',
    'gauge',
    'Messages sent in last hour',
    stats.rateLimiter.lastHour
  );
  addMetric(
    'antiban_rate_last_day',
    'gauge',
    'Messages sent in last day',
    stats.rateLimiter.lastDay
  );
  addMetric(
    'antiban_rate_limit_per_minute',
    'gauge',
    'Rate limit per minute',
    stats.rateLimiter.limits.perMinute
  );
  addMetric(
    'antiban_rate_limit_per_hour',
    'gauge',
    'Rate limit per hour',
    stats.rateLimiter.limits.perHour
  );
  addMetric(
    'antiban_rate_limit_per_day',
    'gauge',
    'Rate limit per day',
  ...
  addMetric(
    ...
    stats.rateLimiter.knownChats
  );

  // Reply ratio gauges (optional)
  if (stats.replyRatio) {
    addMetric(
      'antiban_contacts_on_cooldown',
      'gauge',
      'Contacts currently on reply cooldown',
      stats.replyRatio.contactsOnCooldown
    );
  } else {
  ...
  if (stats.contactGraph) {
    addMetric(
      'antiban_known_contacts',
      'gauge',
      'Number of known contacts',
      stats.contactGraph.knownContacts
    );
    addMetric(
      'antiban_pending_handshakes',
      'gauge',
      'Pending handshakes',
      stats.contactGraph.pendingHandshakes
    );
  } else {
  ...
  if (stats.retryTracker) {
    addMetric(
      'antiban_retry_spirals',
      'gauge',
      'Number of retry spirals detected',
      stats.retryTracker.spiralsDetected
    );
    addMetric(
      'antiban_active_retries',
      'gauge',
      'Currently active retries',
      stats.retryTracker.activeRetries
    );
  } else {
  ...
  if (stats.reconnectThrottle) {
    addMetric(
      'antiban_reconnect_throttled',
      'gauge',
      'Whether reconnect is currently throttled (0=no, 1=yes)',
      stats.reconnectThrottle.isThrottled ? 1 : 0
    );
    addMetric(
      'antiban_lifetime_reconnects',
      'gauge',
      'Total reconnects in session lifetime',
      stats.reconnectThrottle.lifetimeReconnects
    );
  } else {
...
export function createMetricsHandler(
  ...
  return {
     ...
     */
    handle: (_req: unknown, res: { setHeader: (name: string, value: string) => void; end: (data: string) => void }) => {
 ...
 */
export function createPeriodicExporter(
  getStats: () => AntiBanStats,
  config: PeriodicExporterConfig
): PeriodicExporterHandle {
