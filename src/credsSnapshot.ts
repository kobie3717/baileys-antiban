...
export interface CredsSnapshot {
  ...
  list(): Promise<{ path: string; takenAt: Date; size: number }[]>;
}

const noop = () => {};

export function credsSnapshot(config: CredsSnapshotConfig): CredsSnapshot {
  const {
    credsPath,
    snapshotDir = path.join(path.dirname(credsPath), '.snapshots'),
    keep = 3,
    logger = {},
  } = config;
  ...
  async function list(): Promise<{ path: string; takenAt: Date; size: number }[]> {
    try {
  ...
  async function restore(snapshotPath: string): Promise<boolean> {
    try {
