...
function findBaileysDir(candidates: string[]): string | null {
...
function findMakeWASocketFile(baileyDir: string): { file: string; format: 'esm' | 'cjs' } | null {
  ...
  if (fs.existsSync(libDir)) {
    const jsFiles = fs
      .readdirSync(libDir, { recursive: true } as any)
      .filter((f: any) => typeof f === 'string' && f.endsWith('.js'))
      .map((f: any) => path.join(libDir, f));
...
function buildCjsPatch(opts: { preset: string; minDelay: number; maxDelay: number; typing: boolean }): string {
  return `
${PATCH_MARKER_START}
...
function buildEsmPatch(
  source: string,
  opts: { preset: string; minDelay: number; maxDelay: number; typing: boolean }
): string {
  // Strategy A: function declaration — rename and re-export wrapped version
  if (source.includes('export function makeWASocket')) {
    const patched = source.replace(
      /export function makeWASocket\b/,
      'function _antiban_makeWASocket_orig'
    );
    return (
      patched +
      `
...
`
  ...
  if (source.match(/export\s+(const|let)\s+makeWASocket\s*=/)) {
    const patched = source.replace(
      /export\s+(const|let)\s+makeWASocket\s*=/,
      'const _antiban_makeWASocket_orig ='
    );
    return (
      patched +
      `
...
export function applyPatch(opts: PatchOptions): PatchResult {
  const {
    preset = 'conservative',
    minDelay = 1500,
    maxDelay = 4000,
  ...
  if (!baileyDir) {
    return {
      ...
      format: 'cjs',
      message:
        'Could not find Baileys package. Use --path <dir> to specify its location.\n' +
  ...
  if (!found) {
    return {
      ...
      format: 'cjs',
      message:
        `Found Baileys at ${baileyDir} but could not locate makeWASocket definition.\n` +
...
export function unpatchFile(targetFile: string): { success: boolean; message: string } {
