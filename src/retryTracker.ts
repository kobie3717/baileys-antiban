...
export type RetryReason =
  ...
  | 'bad_mac'
  | 'decryption_failure'
  | 'server_error_463'   // rate limit / cooldown
  | 'server_error_429'   // too many requests
  | 'timeout'
  | 'no_route'           // peer offline/unreachable
  | 'node_malformed'
  | 'unknown';
...
export interface RetryTrackerConfig {
  ...
  spiralThreshold?: number;     // same message ID retried >N times = spiral, drop. default 3
...
export interface RetryStats {
  ...
  activeRetries: number;  // messages currently in retry state
}
...
const DEFAULT_CONFIG: Required<RetryTrackerConfig> = {
  enabled: false,
  maxRetries: 5,
  spiralThreshold: 3,
  onSpiral: () => {},
...
export class RetryReasonTracker {
  private config: Required<RetryTrackerConfig>;
  private retries = new Map<string, RetryRecord>();
  private totalRetries = 0;
  private reasonCounts: Record<RetryReason, number> = {
    no_session: 0,
    invalid_key: 0,
    bad_mac: 0,
  ...
  classify(err: any): RetryReason {
    ...
    if (errorMsg.includes('no route') || errorMsg.includes('unreachable') || errorMsg.includes('offline')) return 'no_route';
   ...
   */
  private recordRetry(msgId: string, reason: RetryReason): void {
    const now = Date.now();
