...
const DEFAULT_CONFIG: Required<ReputationVoucherConfig> = {
  maxVouchesPerWeek: 5,
...
export class ReputationVoucher {
  private config: Required<ReputationVoucherConfig>;
  private vouchers: Map<string, VouchingAccount> = new Map();
  private targets: Map<string, VouchTarget> = new Map();
  private conversations: VouchConversation[] = [];
   ...
   */
  registerVoucher(
    account: Omit<VouchingAccount, 'vouchesThisWeek' | 'totalVouches' | 'failedVouches' | 'strikes'>
  ): void {
    // Validate account meets minimum requirements
    if (account.trustScore < this.config.minVoucherTrustScore) {
      throw new Error(
        `Voucher trust score ${account.trustScore} below minimum ${this.config.minVoucherTrustScore}`
      );
    }
    if (account.accountAgeDays < this.config.minVoucherAgeDays) {
      throw new Error(
        `Voucher account age ${account.accountAgeDays} days below minimum ${this.config.minVoucherAgeDays} days`
      );
    }
   ...
   */
  targetQualifies(jid: string): { qualified: boolean; reason?: string; eventsNeeded?: number } {
  ...
  getAvailableVoucher(): VouchingAccount | null {
    ...
    const eligible: VouchingAccount[] = [];
  ...
  planVouchConversation(voucherJid: string, targetJid: string): VouchConversation {
    ...
    const conversation: VouchConversation = {
      targetJid,
      voucherJid,
  ...
  recordVouchOutcome(targetJid: string, success: boolean): void {
    ...
    const conversation = this.conversations.find(
      (c) => c.targetJid === targetJid && c.voucherJid === target.vouchedBy
    );
  ...
  calculateWarmupCredit(targetJid: string): number {
    ...
    const conversation = this.conversations.find(
      (c) => c.targetJid === targetJid && c.success
    );
    if (!conversation) {
      return 0;
    }
